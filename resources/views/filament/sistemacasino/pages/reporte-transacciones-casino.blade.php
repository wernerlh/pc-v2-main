<!-- filepath: resources/views/filament/sistemacasino/pages/reporte-transacciones-casino.blade.php -->
<x-filament-panels::page>
    ...
    <div class="space-y-6">
        <div class="p-6 bg-white rounded-xl shadow dark:bg-gray-800 no-print">
            <h2 class="text-xl font-bold mb-4">Filtrar Transacciones Casino Físico</h2>
        ...
        @if(isset($data['fecha_inicio']) && count($registros) > 0)
            <div id="seccion-resultados" class="p-6 bg-white rounded-xl shadow dark:bg-gray-800">
                ...
                <div class="mb-4 print-block">
                    <h1 class="text-2xl font-bold text-center">Reporte de Transacciones en Casino Físico</h1>
                    ...
                    @if(isset($data['sucursal_id']))
                        <p class="text-center text-gray-500">
                            Sucursal: {{ \App\Models\Sucursales::find($data['sucursal_id'])->nombre }}
                        </p>
                    @endif
                    @if(isset($data['tipo']) && $data['tipo'] !== 'todos')
                        <p class="text-center text-gray-500">
                            Tipo: {{ $data['tipo'] === 'deposito' ? 'Depósitos' : 'Retiros' }}
                        </p>
                    @endif
                </div>
                ...
                <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4 totales-separados">
                    <div class="p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
                        <div class="flex justify-between items-center">
                            <span class="font-bold text-lg text-green-700 dark:text-green-400">Total Depósitos en Casino:</span>
                    ...
                    <div class="p-4 bg-amber-50 dark:bg-amber-900/20 rounded-lg">
                        <div class="flex justify-between items-center">
                            <span class="font-bold text-lg text-amber-700 dark:text-amber-400">Total Retiros en Casino:</span>
                            <span class="font-bold text-xl text-amber-700 dark:text-amber-400">S/ {{ number_format($totalRetiros, 2) }}</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        window.onbeforeprint = function() {
            ...
            if (!document.getElementById('print-header')) {
                ...
                const tipoTexto = tipoSelect ? 
                    (tipoSelect.value === 'deposito' ? 'Depósitos' : 
                     tipoSelect.value === 'retiro' ? 'Retiros' : 'Transacciones') : 
                    'Transacciones';
                
                // Obtener información del cliente y sucursal
                ...
                const sucursalSelect = document.querySelector('select[name="data.sucursal_id"]');
                const sucursalNombre = sucursalSelect && sucursalSelect.selectedIndex > 0 ? 
                    sucursalSelect.options[sucursalSelect.selectedIndex].text : 'Todas las sucursales';
